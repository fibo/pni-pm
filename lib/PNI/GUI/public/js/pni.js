...
PNI.Scenario = function(arg) { 
    ...
    this.uri = '/scenario/'+this.id;

    // TODO vedi se esiste lo scenario e renderizzalo, altrimenti chiami
    // scenario/create
    ...
    this.inEditor = new PNI.InEditor();
...
PNI.InEditor = function(arg) {

    this.$container = $j("#in-editor");

}

PNI.InEditor.prototype = {
...
PNI.Slot = function(arg) {
    ...
    this.uri = '/scenario/'+this.scenario.id+'/node/'+this.node.id;
...
PNI.In = function(arg) {
    ...
    this.box.on("click",function() {
        ...
        if( typeof that.scenario.semiEdge != "undefined" ) {
            ...
            // Turn semiEdge into an edge adding its target.
            that.edge = that.scenario.semiEdge;
            that.scenario.semiEdge = undefined;
            that.edge.target = that;

            var requestData = {
                scenario_id: that.scenario.id,
                source_node_id: that.edge.source.node.id,
                source_slot_id: that.edge.source.id,
                target_node_id: that.edge.target.node.id,
                target_slot_id: that.edge.target.id
            };
            ...
            $j.ajax({
                ...
                success: function(responseData) {
                    ...
                    that.scenario.edges.push(that.edge);
                    that.stage.add(that.edge.layer);
