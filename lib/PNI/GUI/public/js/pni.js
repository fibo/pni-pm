...
PNI.Scenario = function(arg) { 
    ...
    .keypress( function( evt ) {
        if( evt.charCode == 36 ) {
            console.log("TODO: create scalar");
        }
        if( evt.charCode == 37 ) {
            console.log("TODO: create hash");
        }
        if( evt.charCode == 64 ) {
            console.log("TODO: create array");
        }
    });
}
...
PNI.Scenario.prototype = {
    ...
    addNode: function(requestData) { //[
        ...
        requestData.scenario_id = this.id;
    ...
    updateView: function() {
        ...
        $j.getJSON(controllerRoute,requestData, function(responseData){
            ...
            for ( var i in responseData.edges ) {
                ...
                arg.source = PNI.elemById[arg.source_id];
                arg.target = PNI.elemById[arg.target_id];
                arg.scenario = that;
                arg.stage = that.stage;
                arg.layer = that.layer;
                console.log(arg);
...
PNI.NodeSelector = function(arg) {
    ...
    this.$input.keypress( function(evt) {
        if( evt.charCode == 13 ) {
            console.log("ENTER:"+that.$input.val());
...
PNI.Node = function(arg) {
    ...
    this.label = arg.label || 'FIXME';
    this.height = PNI.Theme.nodeHeight;
    this.width = Math.max( this.label.length * PNI.Theme.fontSize, 3 * PNI.Theme.slotSide );
