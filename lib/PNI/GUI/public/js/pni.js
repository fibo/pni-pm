...
/*
            ...
            slotArg.x = slotX;
            slotArg.y = slotY;
            slotArg.stage = this.stage;
*/

            // TODO order si potrebbe ricavare dal node, ins e id dello slot
            var slot = new PNI.In({node:this,order:i});
            console.log(slot);
            this.ins.push(slot);
...
PNI.Slot = function(arg) {
    ...
    this.box.on("mouseover",function(){ that.showInfo(); });
...
PNI.Out.prototype = { 
    ...
    onClick: function() { 
        ...
        arg.startX = this.box.x;
        arg.startY = this.box.y;
        arg.endX = this.box.x-30;
        arg.endY = this.box.y+100;
        arg.stage = this.stage;

        var semiEdge = new PNI.SemiEdge(arg);

        this.stage.add(semiEdge.layer);
...
PNI.Edge = function(arg) {
    ...
    // TODO usa custom shape Kinetic.Shape
...
PNI.SemiEdge = function(arg) {
    ...
    this.startX = 0; //arg.startX;
    this.startY = 0; //arg.startY;
    this.endX = 100; //arg.endX;
    this.endY = 100; //arg.endY;
/*
    var that = this;

    this.line = new Kinetic.Shape({
        ...
        strokeStyle: PNI.Theme.edgeStrokeStyle,
        lineWidth: PNI.Theme.edgeLineWidth
    });
*/
        var context = this.layer.getContext();
...
PNI.extend(PNI.SemiEdge,PNI.Elem);
