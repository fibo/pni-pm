...
PNI.EventEmitter = function(arg) { //[

    var listenersToEvent = {}

    this.emit = function(eventType,arg) { //[

        var listeners = listenersToEvent[eventType] || []

        for (var i=0; i<listeners.length; i++) {

            listeners[i].doActionForEvent(eventType,arg)
    ...
    this.addListener = function(eventType,listener) { //[
        ...
        if(!listenersToEvent[eventType]) listenersToEvent[eventType] = []

        listenersToEvent[eventType].push(listener)
...
PNI.EventListener = function(arg) { //[

console.log("aaaa"+this)
    var actionForEvent = {}

    this.on = function(eventType,action) { actionForEvent[eventType] = action }

    this.doActionForEvent = function(eventType,arg) { 

        var action = actionForEvent[eventType]
...
PNI.CanvasItem = function(arg) { //[
    ...
    PNI.EventEmitter.call(this,arg)
...
console.log("aaaa"+arg)
    PNI.EventListener.call(this,arg)
...
PNI.Window = function(arg) { //[
    ...
    // emitter
    var r = canvas.addItem("rectangle",{centerX:100,centerY:100,height:10,width:10})
    // listener
    var w = PNI.CanvasWidget({canvas:canvas})
    console.log(w)
    w.on("click",function(){console.log("ok")})
    r.addListener("click",w)
